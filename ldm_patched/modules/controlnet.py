    ...
    def get_control(self, x_noisy, t, cond, batched_number, transformer_options):
        ...
        if (
            ...
            if self.vae is not None:
                ...
                if self.latent_format is not None:
                    raise ValueError(
                        "This Controlnet needs a VAE but none was provided, please use a different ControlNetApply node with a VAE input."
                    )
            self.cond_hint = ldm_patched.modules.utils.common_upscale(
            ...
            if self.vae is not None:
                loaded_models = ldm_patched.modules.model_management.loaded_models(
            ...
            if len(self.extra_concat_orig) > 0:
                ...
                for c in self.extra_concat_orig:
                    ...
                    c = ldm_patched.modules.utils.common_upscale(
                        c,
                    ...
                    to_concat.append(
                        ldm_patched.modules.utils.repeat_to_batch_size(
                            c, self.cond_hint.shape[0]
                        )
                    )
...
def controlnet_config(sd, model_options={}):
    ...
    if unet_dtype is None:
        ...
        unet_dtype = ldm_patched.modules.model_management.unet_dtype(
            model_params=-1, supported_dtypes=supported_inference_dtypes
        )
...
def load_controlnet_mmdit(sd, model_options={}):
    ...
    num_blocks = ldm_patched.modules.model_detection.count_blocks(
        new_sd, "joint_blocks.{}."
    )
    for k in sd:
        new_sd[k] = sd[k]
    ...
    if control_latent_channels == 17:  # inpaint controlnet
        concat_mask = True
    control_model = ldm_patched.controlnet.mmdit.ControlNet(
...
def load_controlnet_hunyuandit(controlnet_data, model_options={}):
    ...
    control_model = ldm_patched.modules.ldm.hydit.controlnet.HunYuanControlNet(
...
def load_controlnet_flux_xlabs_mistoline(sd, mistoline=False, model_options={}):
    ...
    control_model = ldm_patched.ldm.flux.controlnet.ControlNetFlux(
        mistoline=mistoline,
        operations=operations,
        device=offload_device,
        dtype=unet_dtype,
...
def convert_mistoline(sd):
    return ldm_patched.modules.utils.state_dict_prefix_replace(
...
def load_controlnet_state_dict(state_dict, model=None, model_options={}):
    ...
    if unet_dtype is None:
        ...
        if supported_inference_dtypes is None:
            supported_inference_dtypes = [
                ldm_patched.modules.model_management.unet_dtype()
            ]
        if weight_dtype is not None:
            supported_inference_dtypes.append(weight_dtype)
        unet_dtype = ldm_patched.modules.model_management.unet_dtype(
    ...
    controlnet_config["device"] = (
        ldm_patched.modules.model_management.unet_offload_device()
    )
    controlnet_config.pop("out_channels")
...
def load_controlnet(ckpt_path, model=None, model_options={}):
    ...
    cnet = load_controlnet_state_dict(
        ldm_patched.modules.utils.load_torch_file(ckpt_path, safe_load=True),
        model=model,
        model_options=model_options,
    )
    if cnet is None:
...
def load_t2i_adapter(t2i_data, model_options={}):  # TODO: model_options
    ...
    if "body.0.in_conv.weight" in keys:
        ...
        model_ad = ldm_patched.ldm.cascade.controlnet.ControlNet(
