...
def detect_unet_config(state_dict, key_prefix):
    ...
    if (
        "clf.1.weight" in state_dict_keys
        or "{}clf.1.weight".format(key_prefix) in state_dict_keys
    ):
        ...
        if (
            text_mapper_name in state_dict_keys
            or text_mapper_name_prefixed in state_dict_keys
        ):
            print("Detected stage c")
            unet_config["stable_cascade_stage"] = "c"
            w = state_dict.get(
                text_mapper_name, state_dict.get(text_mapper_name_prefixed)
            )
            ...
            "clip_mapper.weight" in state_dict_keys
            or "{}clip_mapper.weight".format(key_prefix) in state_dict_keys
        ):
            print("Detected stage b")
            unet_config["stable_cascade_stage"] = "b"
            down_block_key = "down_blocks.1.0.channelwise.0.weight"
            down_block_key_prefixed = "{}down_blocks.1.0.channelwise.0.weight".format(
                key_prefix
            )
...
def convert_diffusers_mmdit(state_dict, output_prefix=""):
    ...
    if "joint_transformer_blocks.0.attn.add_k_proj.weight" in state_dict:  # AuraFlow
        ...
        sd_map = ldm_patched.modules.utils.auraflow_to_diffusers(
        ...
        sd_map = ldm_patched.modules.utils.flux_to_diffusers(
            {
                "depth": depth,
        ...
        sd_map = ldm_patched.modules.utils.mmdit_to_diffusers(
