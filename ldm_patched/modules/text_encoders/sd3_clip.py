...
class T5XXLModel(sd1_clip.SDClipModel):
    def __init__(
        ...
        super().__init__(
            ...
            model_class=ldm_patched.modules.text_encoders.t5.T5,
...
class SD3Tokenizer:
    def __init__(self, embedding_directory=None, tokenizer_data={}):
        clip_l_tokenizer_class = tokenizer_data.get(
            "clip_l_tokenizer_class", sd1_clip.SDTokenizer
        )
...
class SD3ClipModel(torch.nn.Module):
    def __init__(
        ...
        if t5:
            dtype_t5 = ldm_patched.modules.model_management.pick_weight_dtype(
        ...
        if len(token_weight_pairs_g) > 0 or len(token_weight_pairs_l) > 0:
            if self.clip_l is not None:
                ...
                l_pooled = torch.zeros(
                    (1, 768),
                    device=ldm_patched.modules.model_management.intermediate_device(),
                )
