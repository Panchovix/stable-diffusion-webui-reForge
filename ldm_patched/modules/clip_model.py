...
class CLIPEmbeddings(torch.nn.Module):
    ...
    def forward(self, input_tokens, dtype=torch.float32):
        return self.token_embedding(
            input_tokens, out_dtype=dtype
        ) + ldm_patched.modules.ops.cast_to(
...
class CLIPVisionEmbeddings(torch.nn.Module):
    ...
    def forward(self, pixel_values):
        ...
        return torch.cat(
            [
                ldm_patched.modules.ops.cast_to_input(
                    self.class_embedding, embeds
                ).expand(pixel_values.shape[0], 1, -1),
                embeds,
            ],
            dim=1,
        ) + ldm_patched.modules.ops.cast_to_input(
            self.position_embedding.weight, embeds
        )
