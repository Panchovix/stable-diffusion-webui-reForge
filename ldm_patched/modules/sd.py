...
def load_lora_for_models(
    ...
    if loaded_keys_unet or loaded_keys_clip:
        ...
        print(
            f"[LORA] Loaded {filename} for {model_flag} with {total_loaded_keys} keys (UNet: {len(loaded_keys_unet)}, CLIP: {len(loaded_keys_clip)}) at weight {strength_clip}"
        )
        ...
        if shared.opts.cond_stage_model_device_compatibility_check:
            for dt in self.cond_stage_model.dtypes:
                if not model_management.supports_cast(load_device, dt):
                    ...
                    print(
                        f"Conditional stage model dtype {dt} not supported. Falling back to {offload_device}."
                    )
                    break
        else:
            logging.info(
                "Conditional stage model device compatibility check is disabled."
            )
...
class VAE:
    ...
    def decode_tiled_1d(self, samples, tile_x=128, overlap=32):
        ...
        return ldm_patched.modules.utils.tiled_scale_multidim(
            samples,
            decode_fn,
            tile=(tile_x,),
            overlap=overlap,
    ...
    def decode_tiled_3d(
        ...
        return self.process_output(
            ldm_patched.modules.utils.tiled_scale_multidim(
                samples,
                decode_fn,
        ...
        try:
            ...
            for x in range(0, pixel_samples.shape[0], batch_number):
                ...
                samples[x : x + batch_number] = (
                    self.first_stage_model.encode(pixels_in, regulation)
                    .to(self.output_device)
                    .float()
                )
...
def load_text_encoder_state_dicts(
    ...
    for i in range(len(clip_data)):
        if "transformer.resblocks.0.ln_1.weight" in clip_data[i]:
            clip_data[i] = ldm_patched.modules.utils.clip_text_transformers_convert(
                clip_data[i], "", ""
            )
        else:
    ...
    if len(clip_data) == 1:
        ...
        if te_model == TEModel.CLIP_G:
            if clip_type == CLIPType.STABLE_CASCADE:
                ...
                clip_target.clip = ldm_patched.modules.text_encoders.sd3_clip.sd3_clip(
                    clip_l=False, clip_g=True, t5=False
                )
                clip_target.tokenizer = (
                    ldm_patched.modules.text_encoders.sd3_clip.SD3Tokenizer
                )
            else:
            ...
            clip_target.tokenizer = (
                ldm_patched.modules.text_encoders.sd2_clip.SD2Tokenizer
            )
            ...
            clip_target.clip = ldm_patched.modules.text_encoders.sd3_clip.sd3_clip(
                ...
                clip_g=False,
                t5=True,
                dtype_t5=t5xxl_weight_dtype(clip_data),
            )
            clip_target.tokenizer = (
                ldm_patched.modules.text_encoders.sd3_clip.SD3Tokenizer
            )
            ...
            clip_target.tokenizer = (
                ldm_patched.modules.text_encoders.aura_t5.AuraT5Tokenizer
            )
        elif te_model == TEModel.T5_BASE:
            clip_target.clip = ldm_patched.modules.text_encoders.sa_t5.SAT5Model
            clip_target.tokenizer = (
                ldm_patched.modules.text_encoders.sa_t5.SAT5Tokenizer
            )
        else:
        ...
        if clip_type == CLIPType.SD3:
            ...
            clip_target.tokenizer = (
                ldm_patched.modules.text_encoders.hydit.HyditTokenizer
            )
        elif clip_type == CLIPType.FLUX:
            clip_target.clip = ldm_patched.modules.text_encoders.flux.flux_clip(
                dtype_t5=t5xxl_weight_dtype(clip_data)
            )
    ...
    if "parameterization" in model_config_params:
        if model_config_params["parameterization"] == "v":
            ...
            class ModelSamplingAdvanced(
                ...
                ldm_patched.modules.model_sampling.V_PREDICTION,
            ):
                pass
...
def load_diffusion_model_state_dict(
    ...
    diffusion_model_prefix = model_detection.unet_prefix_from_state_dict(sd)
    temp_sd = ldm_patched.modules.utils.state_dict_prefix_replace(
