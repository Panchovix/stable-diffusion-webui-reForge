...
fpunet_group.add_argument(
    "--unet-in-fp16", action="store_true", help="Store unet weights in fp16."
)
fpunet_group.add_argument(
...
fpunet_group.add_argument(
    "--unet-in-fp8-e5m2", action="store_true", help="Store unet weights in fp8_e5m2."
)
...
fpvae_group.add_argument(
    "--vae-in-fp32", action="store_true", help="Run the VAE in full precision fp32."
)
fpvae_group.add_argument(
    "--vae-in-bf16", action="store_true", help="Run the VAE in bf16."
)
...
fpte_group.add_argument(
    "--clip-in-fp16", action="store_true", help="Store text encoder weights in fp16."
)
fpte_group.add_argument(
    "--clip-in-fp32", action="store_true", help="Store text encoder weights in fp32."
)
...
parser.add_argument(
    "--torch-compile",
    action="store_true",
    help="Enable torch.compile for potential speedups",
)
parser.add_argument(
...
parser.add_argument(
    "--torch-compile-epilogue-fusion",
    action="store_true",
    help="Enable epilogue fusion (requires max-autotune)",
)
parser.add_argument(
    "--torch-compile-max-autotune", action="store_true", help="Enable max autotune"
)
parser.add_argument(
    "--torch-compile-fallback-random",
    action="store_true",
    help="Enable fallback random",
)
parser.add_argument(
    "--torch-compile-shape-padding", action="store_true", help="Enable shape padding"
)
parser.add_argument(
    "--torch-compile-cudagraphs", action="store_true", help="Enable CUDA graphs"
)
parser.add_argument("--torch-compile-trace", action="store_true", help="Enable tracing")
parser.add_argument(
    "--torch-compile-graph-diagram", action="store_true", help="Enable graph diagram"
)
...
parser.add_argument(
    ...
    action="store_true",
    help="Enable FP16 accumulation in cuBLAS operations",
)
...
vram_group.add_argument(
    "--always-no-vram", action="store_true", help="When lowvram isn't enough."
)
vram_group.add_argument(
    "--always-cpu", action="store_true", help="To use the CPU for everything (slow)."
)
...
parser.add_argument(
    ...
    action="store_true",
    help="Force reForge to agressively offload to regular ram instead of keeping models in vram when it can.",
)
parser.add_argument(
...
cm_group.add_argument(
    "--cuda-malloc", action="store_true", help="Enable cudaMallocAsync"
)
cm_group.add_argument(
...
parser.add_argument(
    "--disable-server-log", action="store_true", help="Don't print server output."
)
parser.add_argument(
    "--debug-mode", action="store_true", help="Enables more debug prints."
)
parser.add_argument(
