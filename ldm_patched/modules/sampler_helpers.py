...
def get_additional_models(conds, dtype):
    ...
    hook_models = [
        x.model for x in hooks.get(ldm_patched.hooks.EnumHookType.AddModels, {}).keys()
    ]
...
def prepare_model_patcher(model: "ModelPatcher", conds, model_options: dict):
    ...
    model_options["transformer_options"]["wrappers"] = (
        ldm_patched.modules.patcher_extension.copy_nested_dicts(model.wrappers)
    )
    model_options["transformer_options"]["callbacks"] = (
        ldm_patched.modules.patcher_extension.copy_nested_dicts(model.callbacks)
    )
    # register hooks on model/model_options
    model.register_all_hook_patches(
        hooks, ldm_patched.hooks.EnumWeightTarget.Model, model_options
    )
