...
def get_sigmas_ays_32steps(n, sigma_min, sigma_max, is_sdxl=False, device="cpu"):
    ...
    if is_sdxl:
        sigmas = [
            14.61500000000000000,
            11.14916180000000000,
            8.505221270000000000,
            6.488271510000000000,
            5.437074020000000000,
            4.603986190000000000,
            3.898547040000000000,
            3.274074570000000000,
            2.743965270000000000,
            2.299686590000000000,
            1.954485140000000000,
            1.671087150000000000,
            1.428781520000000000,
            1.231810090000000000,
            1.067896490000000000,
            0.925794430000000000,
            0.802908860000000000,
            0.696601210000000000,
            0.604369030000000000,
            0.528525520000000000,
            0.467733440000000000,
            0.413933790000000000,
            0.362581860000000000,
            0.310085170000000000,
            0.265189250000000000,
            0.223264610000000000,
            0.176538770000000000,
            0.139591920000000000,
            0.105873810000000000,
            0.055193690000000000,
            0.028773340000000000,
            0.015000000000000000,
        ]
    else:
        sigmas = [
            14.61500000000000000,
            11.23951352000000000,
            8.643630810000000000,
            6.647294240000000000,
            5.572508620000000000,
            4.716485460000000000,
            3.991960650000000000,
            3.519560900000000000,
            3.134904660000000000,
            2.792287880000000000,
            2.487736280000000000,
            2.216638650000000000,
            1.975083510000000000,
            1.779317200000000000,
            1.614753350000000000,
            1.465409530000000000,
            1.314849000000000000,
            1.166424970000000000,
            1.034755470000000000,
            0.915737440000000000,
            0.807481690000000000,
            0.712023610000000000,
            0.621739000000000000,
            0.530652020000000000,
            0.452909600000000000,
            0.374914550000000000,
            0.274618190000000000,
            0.201152900000000000,
            0.141058730000000000,
            0.066828810000000000,
            0.031661210000000000,
            0.015000000000000000,
        ]
...
def cosine_scheduler(n, sigma_min, sigma_max, device="cpu"):
    ...
    if n == 1:
        ...
        for x in range(n):
            p = x / (n - 1)
            C = sigma_min + 0.5 * (sigma_max - sigma_min) * (
                1 - math.cos(math.pi * (1 - p**0.5))
            )
            sigmas[x] = C
...
def phi_scheduler(n, sigma_min, sigma_max, device="cpu"):
    ...
    if n == 1:
        ...
        phi = (1 + 5**0.5) / 2
        for x in range(n):
            sigmas[x] = sigma_min + (sigma_max - sigma_min) * (
                (1 - x / (n - 1)) ** (phi * phi)
            )
    return torch.cat([sigmas, sigmas.new_zeros([1])])
...
def get_sigmas_karras_dynamic(n, sigma_min, sigma_max, device="cpu"):
    ...
    for i in range(n):
        sigmas[i] = (max_inv_rho + ramp[i] * (min_inv_rho - max_inv_rho)) ** (
            math.cos(i * math.tau / n) * 2 + rho
        )
    return torch.cat([sigmas, sigmas.new_zeros([1])])
...
def get_sigmas_sinusoidal_sf(n, sigma_min, sigma_max, sf=3.5, device="cpu"):
    x = torch.linspace(0, 1, n, device=device)
    sigmas = (
        sigma_min + (sigma_max - sigma_min) * (1 - torch.sin(torch.pi / 2 * x))
    ) / sigma_max
    sigmas = sigmas**sf
...
def get_sigmas_invcosinusoidal_sf(n, sigma_min, sigma_max, sf=3.5, device="cpu"):
    x = torch.linspace(0, 1, n, device=device)
    sigmas = (
        sigma_min + (sigma_max - sigma_min) * (0.5 * (torch.cos(x * math.pi) + 1))
    ) / sigma_max
    sigmas = sigmas**sf
    sigmas = sigmas * sigma_max
    return torch.cat([sigmas, sigmas.new_zeros([1])])
...
def get_sigmas_react_cosinusoidal_dynsf(n, sigma_min, sigma_max, sf=2.15, device="cpu"):
    x = torch.linspace(0, 1, n, device=device)
    sigmas = (
        sigma_min + (sigma_max - sigma_min) * (torch.cos(x * (torch.pi / 2)))
    ) / sigma_max
...
class ODEFunction:
    def __init__(
        self, model, t_min, t_max, n_steps, is_adaptive, extra_args=None, callback=None
    ):
        self.model = model
        ...
        if is_adaptive:
            self.pbar = tqdm(total=100, desc="solve", unit="%", leave=False, position=1)
        else:
            self.pbar = tqdm(total=n_steps, desc="solve", leave=False, position=1)
...
def sample_dpmpp_2s_ancestral_cfg_pp(
    ...
    extra_args["model_options"] = (
        ldm_patched.modules.model_patcher.set_model_options_post_cfg_function(
...
class DPMSolver(nn.Module):
    ...
    def dpm_solver_fast(
        self, x, t_start, t_end, nfe, eta=0.0, s_noise=None, noise_sampler=None
    ):
    ...
    def dpm_solver_adaptive(
        ...
        s_noise = (
            modules.shared.opts.dpm_adaptive_s_noise if s_noise is None else s_noise
        )
        noise_sampler = (
            default_noise_sampler(x, seed=self.extra_args.get("seed", None))
            if noise_sampler is None
...
@torch.no_grad()
def sample_dpmpp_2s_ancestral_cfg_pp_dyn(
    model,
    x,
    sigmas,
    extra_args=None,
    ...
    s_noise = (
        modules.shared.opts.dpmpp_2s_ancestral_dyn_s_noise
        if s_noise is None
        else s_noise
    )
    extra_args = {} if extra_args is None else extra_args
...
@torch.no_grad()
def sample_dpmpp_2s_ancestral_cfg_pp_intern(
    model,
    x,
    sigmas,
    extra_args=None,
    ...
    s_noise = (
        modules.shared.opts.dpmpp_2s_ancestral_intern_s_noise
        if s_noise is None
        else s_noise
    )
    extra_args = {} if extra_args is None else extra_args
    ...
    x = (sigma_fn(t_next) / sigma_fn(t)) * (x + (den - temp[0])) - (
        -h
    ).expm1() * denoised_2
    large_denoised = x
    ...
    for i in trange(len(sigmas) - 2, disable=disable):
        ...
        if sigma_down == 0:
            ...
            if mergefactor == 1:
                ...
                up_temp = nn.functional.interpolate(
                    temp[0], scale_factor=2, mode="area"
                )
                x_2 = (sigma_fn(s) / sigma_fn(t)) * (x + (up_den - up_temp)) - (
                    -h * r
                ).expm1() * up_den
            else:
                up_den = nn.functional.interpolate(
                    denoised, scale_factor=2, mode="area"
                )
            ...
            denoised_2 = model(x_2, sigma_fn(s) * s_in, **extra_args)
            x = (sigma_fn(t_next) / sigma_fn(t)) * (x + (up_den - temp[0])) - (
                -h
            ).expm1() * denoised_2
...
def sample_dpmpp_sde_cfg_pp(
    ...
    noise_sampler = (
        BrownianTreeNoiseSampler(
            x, sigma_min, sigma_max, seed=extra_args.get("seed", None), cpu=True
        )
        if noise_sampler is None
        else noise_sampler
    )
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            sd, su = get_ancestral_step(sigma_fn(t), sigma_fn(t_next), eta)
            t_next_ = t_fn(sd)
            denoised_d = (1 - fac) * temp[0] + fac * temp[
                0
            ]  # Use temp[0] instead of denoised
...
def sample_dpmpp_3m_sde_cfg_pp(
    ...
    s_noise = (
        modules.shared.opts.dpmpp_3m_sde_cfg_pp_s_noise if s_noise is None else s_noise
    )
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            x = (
                torch.exp(-h_eta) * (x + (denoised - temp[0]))
                + (-h_eta).expm1().neg() * denoised
            )
...
def sample_dpmpp_2m_dy(
    ...
    s_dy_pow = (
        modules.shared.opts.dpmpp_2m_dy_s_dy_pow if s_dy_pow is None else s_dy_pow
    )
    s_extra_steps = (
        modules.shared.opts.dpmpp_2m_dy_s_extra_steps
        if s_extra_steps is None
        else s_extra_steps
    )
    extra_args = {} if extra_args is None else extra_args
    s_in = x.new_ones([x.shape[0]])
...
def sample_dpmpp_2m_sde_dy(
    ...
    s_noise = (
        modules.shared.opts.dpmpp_2m_sde_dy_s_noise if s_noise is None else s_noise
    )
    solver_type = (
        modules.shared.opts.dpmpp_2m_sde_dy_solver_type
        if solver_type is None
        else solver_type
    )
    s_dy_pow = (
        modules.shared.opts.dpmpp_2m_sde_dy_s_dy_pow if s_dy_pow is None else s_dy_pow
    )
    s_extra_steps = (
        modules.shared.opts.dpmpp_2m_sde_dy_s_extra_steps
        if s_extra_steps is None
        else s_extra_steps
    )
    if len(sigmas) <= 1:
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            x = (
                sigmas[i + 1] / sigma_hat * (-eta_h).exp() * x
                + (-h - eta_h).expm1().neg() * denoised
            )
            ...
            if eta:
                x = (
                    x
                    + noise_sampler(sigma_hat, sigmas[i + 1] * (gamma + 1))
                    * sigmas[i + 1]
                    * (-2 * eta_h).expm1().neg().sqrt()
                    * s_noise
                )
...
def sample_dpmpp_3m_sde_dy(
    ...
    s_noise = (
        modules.shared.opts.dpmpp_3m_sde_dy_s_noise if s_noise is None else s_noise
    )
    s_dy_pow = (
        modules.shared.opts.dpmpp_3m_sde_dy_s_dy_pow if s_dy_pow is None else s_dy_pow
    )
    s_extra_steps = (
        modules.shared.opts.dpmpp_3m_sde_dy_s_extra_steps
        if s_extra_steps is None
        else s_extra_steps
    )
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            if eta:
                x = (
                    x
                    + noise_sampler(sigmas[i], sigmas[i + 1] * (gamma + 1))
                    * sigmas[i + 1]
                    * (-2 * h * eta).expm1().neg().sqrt()
                    * s_noise
                )
...
def sample_dpmpp_3m_dy(
    ...
    s_dy_pow = (
        modules.shared.opts.dpmpp_3m_dy_s_dy_pow if s_dy_pow is None else s_dy_pow
    )
    s_extra_steps = (
        modules.shared.opts.dpmpp_3m_dy_s_extra_steps
        if s_extra_steps is None
        else s_extra_steps
    )
...
def dy_sampling_step_cfg_pp(
    x, model, sigma_next, i, sigma, sigma_hat, callback, **extra_args
):
    temp = [0]
...
def sample_euler_dy_cfg_pp(
    ...
    s_churn = (
        modules.shared.opts.euler_dy_cfg_pp_s_churn if s_churn is None else s_churn
    )
    s_tmin = modules.shared.opts.euler_dy_cfg_pp_s_tmin if s_tmin is None else s_tmin
    s_noise = (
        modules.shared.opts.euler_dy_cfg_pp_s_noise if s_noise is None else s_noise
    )
    s_dy_pow = (
        modules.shared.opts.euler_dy_cfg_pp_s_dy_pow if s_dy_pow is None else s_dy_pow
    )
    s_extra_steps = (
        modules.shared.opts.euler_dy_cfg_pp_s_extra_steps
        if s_extra_steps is None
        else s_extra_steps
    )
    extra_args = {} if extra_args is None else extra_args
...
def smea_sampling_step_cfg_pp(
    ...
    x = torch.nn.functional.interpolate(
        input=x, scale_factor=(1.25, 1.25), mode="nearest-exact"
    )
...
def sample_euler_smea_dy_cfg_pp(
    ...
    s_churn = (
        modules.shared.opts.euler_smea_dy_cfg_pp_s_churn if s_churn is None else s_churn
    )
    s_tmin = (
        modules.shared.opts.euler_smea_dy_cfg_pp_s_tmin if s_tmin is None else s_tmin
    )
    s_noise = (
        modules.shared.opts.euler_smea_dy_cfg_pp_s_noise if s_noise is None else s_noise
    )
    s_dy_pow = (
        modules.shared.opts.euler_smea_dy_cfg_pp_s_dy_pow
        if s_dy_pow is None
        else s_dy_pow
    )
    s_extra_steps = (
        modules.shared.opts.euler_smea_dy_cfg_pp_s_extra_steps
        if s_extra_steps is None
        else s_extra_steps
    )
    extra_args = {} if extra_args is None else extra_args
...
def sample_euler_ancestral_dy_cfg_pp(
    ...
    s_noise = (
        modules.shared.opts.euler_ancestral_dy_cfg_pp_s_noise
        if s_noise is None
        else s_noise
    )
    s_dy_pow = (
        modules.shared.opts.euler_ancestral_dy_cfg_pp_s_dy_pow
        if s_dy_pow is None
        else s_dy_pow
    )
    s_extra_steps = (
        modules.shared.opts.euler_ancestral_dy_cfg_pp_s_extra_steps
        if s_extra_steps is None
        else s_extra_steps
    )
    extra_args = {} if extra_args is None else extra_args
...
def sample_dpmpp_2m_dy_cfg_pp(
    ...
    s_noise = (
        modules.shared.opts.dpmpp_2m_dy_cfg_pp_s_noise if s_noise is None else s_noise
    )
    s_dy_pow = (
        modules.shared.opts.dpmpp_2m_dy_cfg_pp_s_dy_pow
        if s_dy_pow is None
        else s_dy_pow
    )
    s_extra_steps = (
        modules.shared.opts.dpmpp_2m_dy_cfg_pp_s_extra_steps
        if s_extra_steps is None
        else s_extra_steps
    )
    """DPM-Solver++(2M)."""
...
@torch.no_grad()
def sample_clyb_4m_sde_momentumized(
    model,
    x,
    sigmas,
    extra_args=None,
    ...
    def momentum_func(
        diff, velocity, timescale=1.0, offset=-momentum / 2.0
    ):  # Diff is current diff, vel is previous diff
        if velocity is None:
            momentum_vel = diff
        else:
            momentum_vel = (
                momentum * (timescale + offset) * velocity
                + (1 - momentum * (timescale + offset)) * diff
            )
        return momentum_vel
        ...
        if sigmas[i + 1] == 0:
            ...
            if h_3 is not None:
                ...
                d1 = (
                    d1_0
                    + (d1_0 - d1_1) * r2 / (r2 + r1)
                    + ((d1_0 - d1_1) * r2 / (r2 + r1) - (d1_1 - d1_2) * r1 / (r0 + r1))
                    * r2
                    / ((r2 + r1) * (r0 + r1))
                )
                d2 = (d1_0 - d1_1) / (r2 + r1) + (
...
class DenoiserModel(Protocol):
    def __call__(
        self, x: FloatTensor, t: FloatTensor, *args, **kwargs
    ) -> FloatTensor: ...
...
def sample_refined_exp_s(
    ...
    vel, vel_2 = None, None
    with tqdm(
        disable=disable, total=len(sigmas) - (1 if denoise_to_zero else 2)
    ) as pbar:
        ...
        if denoise_to_zero:
            ...
            sigma_hat = sigma * (1 + ita)
            x_hat = x + (sigma_hat**2 - sigma**2).sqrt() * eps
            x_next: FloatTensor = model(
                x_hat, sigma.to(x_hat.device).repeat(x_hat.size(0)), **extra_args
            )
            pbar.update()
...
def sample_Kohaku_LoNyu_Yog(
    ...
    s_churn = (
        modules.shared.opts.kohaku_lonyu_yog_s_churn if s_churn is None else s_churn
    )
    s_tmin = modules.shared.opts.kohaku_lonyu_yog_s_tmin if s_tmin is None else s_tmin
    s_noise = (
        modules.shared.opts.kohaku_lonyu_yog_s_noise if s_noise is None else s_noise
    )
...
def sample_kohaku_lonyu_yog_cfg_pp(
    ...
    # Get values from shared options if not provided
    s_churn = (
        modules.shared.opts.kohaku_lonyu_yog_s_cfgpp_churn
        if s_churn is None
        else s_churn
    )
    s_tmin = (
        modules.shared.opts.kohaku_lonyu_yog_s_cfgpp_tmin if s_tmin is None else s_tmin
    )
    s_noise = (
        modules.shared.opts.kohaku_lonyu_yog_s_cfgpp_noise
        if s_noise is None
        else s_noise
    )
...
def res_multistep(
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0 or old_denoised is None:
            ...
            x = (sigma_fn(t_next) / sigma_fn(t)) * x + h * (
