...
class Hook:
    ...
    def should_register(
        ...
        return self.custom_should_register(
            self, model, model_options, target, registered
        )
...
class WeightHook(Hook):
    ...
    def add_hook_patches(
        ...
        if self.need_weight_init:
            key_map = {}
            if target == EnumWeightTarget.Model:
                key_map = ldm_patched.modules.lora.model_lora_keys_unet(
                    model.model, key_map
                )
            else:
            ...
            weights = ldm_patched.modules.lora.load_lora(
                self.weights, key_map, log_missing=False
            )
        else:
            if target == EnumWeightTarget.Model:
...
class WrapperHook(Hook):
    def __init__(
        self, wrappers_dict: dict[str, dict[str, dict[str, list[Callable]]]] = None
    ):
...
class HookGroup:
    ...
    def combine_all_hooks(
        hooks_list: list["HookGroup"], require_count=0
    ) -> "HookGroup":
...
def set_hooks_for_conditioning(cond, hooks: HookGroup, append_hooks=True):
    ...
    return conditioning_set_values_with_hooks(
        cond, {"hooks": hooks}, append_hooks=append_hooks
    )
...
def set_conds_props_and_combine(
    ...
    for c, masked_c in zip(conds, new_conds):
        ...
        masked_c = set_hooks_for_conditioning(
            masked_c, hooks, append_hooks=append_hooks
        )
