...
class Toprow:
    ...
    button_interrogate: ToolButton
    button_deepbooru: ToolButton
    ...
    ui_styles: ui_prompt_styles.UiPromptStyles

    submit_box: gr.Row

    def __init__(self, is_img2img: bool, is_compact=False, id_part=None):
        if id_part is None:
    ...
    def create_inline_toprow_image(self):
        if not self.is_compact:
            return
        if self.submit_box is None:
            raise Exception("Toprow Submit box has not been constructed.")
    ...
    def create_submit_box(self):
        with gr.Row(
            ...
            self.interrupt.click(
                ...
                js='function(){ showSubmitInterruptingPlaceholder("'
                + self.id_part
                + '"); }',
            )
