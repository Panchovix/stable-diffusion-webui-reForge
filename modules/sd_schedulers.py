...
def get_sigmas_karras(n, sigma_min, sigma_max, rho=7.0, device="cpu"):
    ...
    return ldm_patched.k_diffusion.sampling.get_sigmas_karras(
        n, sigma_min, sigma_max, rho, device
    )
...
def get_sigmas_polyexponential(n, sigma_min, sigma_max, device="cpu"):
    ...
    return ldm_patched.k_diffusion.sampling.get_sigmas_polyexponential(
        n, sigma_min, sigma_max, rho, device
    )
...
def get_sigmas_ays_custom(n, sigma_min, sigma_max, device="cpu"):
    try:
        ...
        if n != len(sigmas):
            sigmas = np.interp(
                np.linspace(0, 1, n), np.linspace(0, 1, len(sigmas)), sigmas
            )
...
def turbo_scheduler(n, sigma_min, sigma_max, inner_model, device):
    ...
    timesteps = (
        torch.flip(torch.arange(1, n + 1) * float(1000.0 / n) - 1, (0,))
        .round()
        .long()
        .clip(0, 999)
    )
...
schedulers = [
    ...
    Scheduler(
        "polyexponential",
        "Polyexponential",
        sampling.get_sigmas_polyexponential,
        default_rho=1.0,
    ),
    Scheduler("sinusoidal_sf", "Sinusoidal SF", get_sigmas_sinusoidal_sf),
    Scheduler(
        "invcosinusoidal_sf", "Invcosinusoidal SF", get_sigmas_invcosinusoidal_sf
    ),
    Scheduler(
        "react_cosinusoidal_dynsf",
        "React Cosinusoidal DynSF",
        get_sigmas_react_cosinusoidal_dynsf,
    ),
    ...
    Scheduler(
        "align_your_steps_custom", "Align Your Steps Custom", get_sigmas_ays_custom
    ),
    Scheduler("beta", "Beta", beta_scheduler, need_inner_model=True),
    Scheduler("turbo", "Turbo", turbo_scheduler, need_inner_model=True),
    Scheduler("cosine", "Cosine", cosine_scheduler),
    Scheduler(
        "cosine-exponential blend", "Cosine-exponential Blend", cosexpblend_scheduler
    ),
    ...
    Scheduler(
        ...
        get_align_your_steps_sigmas_GITS,
    ),
