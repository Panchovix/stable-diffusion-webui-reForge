...
def image_grid(imgs, batch_size=1, rows=None):
    ...
    grid = Image.new(
        "RGBA", size=(params.cols * w, params.rows * h), color=grid_background_color
    )
    ...
    def draw_texts(drawing, draw_x, draw_y, lines, initial_fnt, initial_fontsize):
        for line in lines:
            ...
            if parse_version(pillow_version) >= parse_version("10.0.0"):
                ...
                text_width, text_height = drawing.multiline_textbbox(
                    (0, 0), line.text, font=fnt
                )[2:]
                ...
                while (
                    drawing.multiline_textsize(line.text, font=fnt)[0]
                    > line.allowed_width
                    and fontsize > 0
                ):
                    fontsize -= 1
...
class FilenameGenerator:
    ...
    def __init__(self, p, seed, prompt, image, zip=False, basename: str = ""):
        self.p = p
        self.seed = seed
        self.prompt = prompt
        self.image = image
    ...
    def get_vae_filename(self) -> str:
        ...
        file_name = Path(sd_vae.loaded_vae_file)
        # TODO: Unfix this - Ristellise
        split_file_name = file_name.name.split(".")
...
def get_next_sequence_number(directory: Path, basename):
    """
    ...
    if forced_filename is None:
        ...
        if add_number:
            ...
            for i in range(500):
                ...
                fullfn = path / f"{fn}{file_decoration}.{extension}"
                if not os.path.exists(fullfn):
                    break
        else:
            fullfn = path / f"{file_decoration}.{extension}"
    else:
        fullfn = path / f"{forced_filename}.{extension}"
