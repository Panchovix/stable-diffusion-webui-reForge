...
if opts.sd_sampler_cfg_denoiser == "reForge":
    ...
    def pad_cond(tensor, repeats, empty):
        if not isinstance(tensor, dict):
            return torch.cat(
                [tensor, empty.repeat((tensor.shape[0], repeats, 1))], axis=1
            )
    ...
    class CFGDenoiser(torch.nn.Module):
        ...
        def forward(
            self, x, sigma, uncond, cond, cond_scale, s_min_uncond, image_cond, **kwargs
        ):
            ...
            if self.classic_ddim_eps_estimation:
                ...
                x = (
                    x
                    * (
                        ((real_sigma**2.0 + real_sigma_data**2.0) ** 0.5)[
                            :, None, None, None
                        ]
                    )
                )
            ...
            def apply_blend(current_latent, noisy_initial_latent=None):
                ...
                blended_latent = (
                    current_latent * self.nmask + noisy_initial_latent * self.mask
                )
            ...
            if s_min_uncond > 0 and sigma[0] < s_min_uncond and not is_edit_model:
                if self.step % 2 == 0 or shared.opts.s_min_uncond_all:
                    ...
                    print(
                        f"Applying NGMS at step {self.step}: s_min_uncond = {s_min_uncond}, sigma = {sigma[0]}"
                    )
            ...
            control = (
                self.inner_model.inner_model.forge_objects.unet.controlnet_linked_list
            )
            extra_concat_condition = (
                self.inner_model.inner_model.forge_objects.unet.extra_concat_condition
            )
            model_options = (
                self.inner_model.inner_model.forge_objects.unet.model_options
            )
            ...
            uncond_patched = forge_sampler.cond_from_a1111_to_patched_ldm(
                denoiser_params.text_uncond
            )
            cond_patched = forge_sampler.cond_from_a1111_to_patched_ldm_weighted(
            ...
            if isinstance(image_cond_in, torch.Tensor):
                if (
                    ...
                    for i in range(len(uncond_patched)):
                        uncond_patched[i]["model_conds"]["c_concat"] = CONDRegular(
                            image_cond_in
                        )
                    for i in range(len(cond_patched)):
                        cond_patched[i]["model_conds"]["c_concat"] = CONDRegular(
                            image_cond_in
                        )
            ...
            if skip_uncond:
                ...
                denoised = sampling_function(
                    model, x, sigma, None, cond_patched, 1.0, model_options, seed
                )
            else:
                denoised = sampling_function(
                    model,
            ...
            if self.need_last_noise_uncond:
                if skip_uncond:
                    self.last_noise_uncond = torch.zeros_like(
                        x
            ...
            if is_edit_model:
                denoised = self.combine_denoised_for_edit_model(
                    denoised, cond_scale * self.cond_scale_miltiplier
                )
            elif skip_uncond:
            ...
            after_cfg_callback_params = AfterCFGCallbackParams(
                denoised, shared.state.sampling_step, shared.state.sampling_steps
            )
    ...
    class CFGDenoiser(torch.nn.Module):
        ...
        def forward(
            ...
            conds_list, cond = prompt_parser.reconstruct_multicond_batch(
                cond, self.step
            )
            uncond = prompt_parser.reconstruct_cond_batch(uncond, self.step)
            ...
            cond_patched = forge_sampler.cond_from_a1111_to_patched_ldm_weighted(
                denoiser_params.text_cond, conds_list
            )
