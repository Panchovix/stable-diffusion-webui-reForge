...
class AlterSampler(sd_samplers_kdiffusion.KDiffusionSampler):
    ...
    def sample_func(
        self, model, x, sigmas, extra_args=None, callback=None, disable=None
    ):
        if self.sampler_name == "ode_bosh3":
            return self.sample_ode_bosh3(
                model, x, sigmas, extra_args, callback, disable
            )
        elif self.sampler_name == "ode_fehlberg2":
            return self.sample_ode_fehlberg2(
                model, x, sigmas, extra_args, callback, disable
            )
        elif self.sampler_name == "ode_adaptive_heun":
            return self.sample_ode_adaptive_heun(
                model, x, sigmas, extra_args, callback, disable
            )
        elif self.sampler_name == "ode_dopri5":
            return self.sample_ode_dopri5(
                model, x, sigmas, extra_args, callback, disable
            )
        elif self.sampler_name == "ode_custom":
            return self.sample_ode_custom(
                model, x, sigmas, extra_args, callback, disable
            )
        else:
    ...
    def sample_ode_bosh3(
        self, model, x, sigmas, extra_args=None, callback=None, disable=None
    ):
        return k_diffusion_sampling.sample_ode(
            model,
            x,
            sigmas,
    ...
    def sample(
        ...
        return super().sample(
            p, x, conditioning, unconditional_conditioning, steps, image_conditioning
        )
    ...
    def get_sigmas(self, p, steps):
        ...
        use_turbo = (
            self.sampler_name.endswith("_turbo") or self.scheduler_name == "Turbo"
        )
        ...
        try:
            if use_turbo:
                ...
                timesteps = (
                    torch.flip(
                        torch.arange(1, steps + 1) * float(1000.0 / steps) - 1, (0,)
                    )
                    .round()
                    .long()
                    .clip(0, 999)
                ...
                sigmas = calculate_sigmas(
                    self.unet.model.model_sampling,
                    matched_scheduler,
                    steps,
                    is_sdxl=getattr(self.model, "is_sdxl", False),
                )
        except Exception as e:
            print(
                f"Error calculating sigmas for scheduler {matched_scheduler}: {str(e)}"
            )
...
samplers_data_alter = [
    sd_samplers_common.SamplerData(
        "Kohaku_LoNyu_Yog",
        build_constructor(sampler_name="Kohaku_LoNyu_Yog"),
        ["Kohaku_LoNyu_Yog"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "Euler CFG++",
        build_constructor(sampler_name="euler_cfg_pp"),
        ["euler_cfg_pp"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "Euler Ancestral CFG++",
        build_constructor(sampler_name="euler_ancestral_cfg_pp"),
        ["euler_ancestral_cfg_pp"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 2S Ancestral CFG++",
        build_constructor(sampler_name="dpmpp_2s_ancestral_cfg_pp"),
        ["dpmpp_2s_ancestral_cfg_pp"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 2M CFG++",
        build_constructor(sampler_name="dpmpp_2m_cfg_pp"),
        ["dpmpp_2m_cfg_pp"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ SDE CFG++",
        build_constructor(sampler_name="dpmpp_sde_cfg_pp"),
        ["dpmpp_sde_cfg_pp"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 3M SDE CFG++",
        build_constructor(sampler_name="dpmpp_3m_sde_cfg_pp"),
        ["dpmpp_3m_sde_cfg_pp"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "Euler DY CFG++",
        build_constructor(sampler_name="euler_dy_cfg_pp"),
        ["euler_dy_cfg_pp"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "Euler SMEA DY CFG++",
        build_constructor(sampler_name="euler_smea_dy_cfg_pp"),
        ["euler_smea_dy_cfg_pp"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "Euler A DY CFG++",
        build_constructor(sampler_name="euler_ancestral_dy_cfg_pp"),
        ["euler_ancestral_dy_cfg_pp"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 2M DY CFG++",
        build_constructor(sampler_name="dpmpp_2m_dy_cfg_pp"),
        ["dpmpp_2m_dy_cfg_pp"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "RES Multistep",
        build_constructor(sampler_name="res_multistep"),
        ["res_multistep"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "RES Multistep CFG++",
        build_constructor(sampler_name="res_multistep_cfg_pp"),
        ["res_multistep_cfg_pp"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "Gradient Estimation",
        build_constructor(sampler_name="gradient_estimation"),
        ["gradient_estimation"],
        {},
    ),
    ...
    sd_samplers_common.SamplerData(
        "DPM++ 2M DY",
        build_constructor(sampler_name="dpmpp_2m_dy"),
        ["dpmpp_2m_dy"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 3M DY",
        build_constructor(sampler_name="dpmpp_3m_dy"),
        ["dpmpp_3m_dy"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 3M SDE DY",
        build_constructor(sampler_name="dpmpp_3m_sde_dy"),
        ["dpmpp_3m_sde_dy"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DDPM", build_constructor(sampler_name="ddpm"), ["ddpm"], {}
    ),
    sd_samplers_common.SamplerData(
        "HeunPP2", build_constructor(sampler_name="heunpp2"), ["heunpp2"], {}
    ),
    sd_samplers_common.SamplerData(
        "IPNDM", build_constructor(sampler_name="ipndm"), ["ipndm"], {}
    ),
    sd_samplers_common.SamplerData(
        "IPNDM_V", build_constructor(sampler_name="ipndm_v"), ["ipndm_v"], {}
    ),
    sd_samplers_common.SamplerData(
        "DEIS", build_constructor(sampler_name="deis"), ["deis"], {}
    ),
    sd_samplers_common.SamplerData(
        "RES Solver", build_constructor(sampler_name="res_solver"), ["res_solver"], {}
    ),
    sd_samplers_common.SamplerData(
        "ODE (Bosh3)", build_constructor(sampler_name="ode_bosh3"), ["ode_bosh3"], {}
    ),
    sd_samplers_common.SamplerData(
        "ODE (Fehlberg2)",
        build_constructor(sampler_name="ode_fehlberg2"),
        ["ode_fehlberg2"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "ODE (Adaptive Heun)",
        build_constructor(sampler_name="ode_adaptive_heun"),
        ["ode_adaptive_heun"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "ODE (Dopri5)", build_constructor(sampler_name="ode_dopri5"), ["ode_dopri5"], {}
    ),
    sd_samplers_common.SamplerData(
        "ODE Custom", build_constructor(sampler_name="ode_custom"), ["ode_custom"], {}
    ),
    sd_samplers_common.SamplerData(
        "Euler A Turbo",
        build_constructor(sampler_name="euler_ancestral_turbo"),
        ["euler_ancestral_turbo"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 2M Turbo",
        build_constructor(sampler_name="dpmpp_2m_turbo"),
        ["dpmpp_2m_turbo"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 2M SDE Turbo",
        build_constructor(sampler_name="dpmpp_2m_sde_turbo"),
        ["dpmpp_2m_sde_turbo"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "Euler Comfy",
        build_constructor(sampler_name="euler_comfy"),
        ["euler_comfy"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "Euler A Comfy",
        build_constructor(sampler_name="euler_ancestral_comfy"),
        ["euler_ancestral_comfy"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "Heun Comfy", build_constructor(sampler_name="heun_comfy"), ["heun_comfy"], {}
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 2S Ancestral Comfy",
        build_constructor(sampler_name="dpmpp_2s_ancestral_comfy"),
        ["dpmpp_2s_ancestral_comfy"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ SDE Comfy",
        build_constructor(sampler_name="dpmpp_sde_comfy"),
        ["dpmpp_sde_comfy"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 2M Comfy",
        build_constructor(sampler_name="dpmpp_2m_comfy"),
        ["dpmpp_2m_comfy"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 2M SDE Comfy",
        build_constructor(sampler_name="dpmpp_2m_sde_comfy"),
        ["dpmpp_2m_sde_comfy"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 3M SDE Comfy",
        build_constructor(sampler_name="dpmpp_3m_sde_comfy"),
        ["dpmpp_3m_sde_comfy"],
        {},
    ),
    ...
    sd_samplers_common.SamplerData(
        ...
        build_constructor(sampler_name="dpmpp_2s_ancestral_cfg_pp_dyn"),
        ["dpmpp_2s_ancestral_cfg_pp_dyn"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "DPM++ 2S Ancestral CFG++ Intern",
        build_constructor(sampler_name="dpmpp_2s_ancestral_cfg_pp_intern"),
        ["dpmpp_2s_ancestral_cfg_pp_intern"],
        {},
    ),
    sd_samplers_common.SamplerData(
        "Custom Sampler",
        build_constructor(sampler_name="custom_sampler"),
        ["custom_sampler"],
        {},
    ),
