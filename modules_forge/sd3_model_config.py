...
class SD3ModelConfig:
    ...
    def get_model(self, state_dict, prefix="model.diffusion_model.", device=None):
        ...
        return BaseModel(
            shift=shift, state_dict=state_dict, prefix=prefix, device=device
        )

    def process_clip_state_dict(self, state_dict):
        clip_l = ldm_patched.modules.utils.state_dict_prefix_replace(
            state_dict, {"clip_l.": ""}
        )
        clip_g = ldm_patched.modules.utils.state_dict_prefix_replace(
            state_dict, {"clip_g.": ""}
        )
        t5xxl = ldm_patched.modules.utils.state_dict_prefix_replace(
            state_dict, {"t5xxl.": ""}
        )
