...
class CLIP_SD_15_L(FrozenCLIPEmbedderWithCustomWords):
    ...
    def encode_with_transformers(self, tokens):
        ...
        if hasattr(self.wrapped.transformer, "text_model"):
            ...
            outputs = self.wrapped.transformer(
                tokens, intermediate_output=-opts.CLIP_stop_at_last_layers
            )

        if opts.CLIP_stop_at_last_layers > self.minimal_clip_skip:
            z = (
                outputs.hidden_states[-opts.CLIP_stop_at_last_layers]
...
class CLIP_SD_21_H(FrozenCLIPEmbedderWithCustomWords):
    ...
    def encode_with_transformers(self, tokens):
        ...
        if hasattr(self.wrapped.transformer, "text_model"):
            outputs = self.wrapped.transformer(
                tokens, output_hidden_states=self.wrapped.layer == "hidden"
            )
        else:
            outputs = self.wrapped.transformer(
                tokens,
                intermediate_output=self.wrapped.layer_idx
                if self.wrapped.layer == "hidden"
                else None,
            )
        ...
        if opts.CLIP_stop_at_last_layers > self.minimal_clip_skip:
            ...
            z = (
                outputs.hidden_states[self.wrapped.layer_idx]
                if hasattr(outputs, "hidden_states")
                else outputs[1]
            )
            z = self.wrapped.transformer.text_model.final_layer_norm(z)
...
class CLIP_SD_XL_G(FrozenCLIPEmbedderWithCustomWords):
    ...
    def encode_with_transformers(self, tokens):
        ...
        pooled_output = (
            outputs.pooler_output if hasattr(outputs, "pooler_output") else outputs[2]
        )
        text_projection = self.wrapped.text_projection
        pooled_output = (
            pooled_output.float().to(text_projection.device) @ text_projection.float()
        )
