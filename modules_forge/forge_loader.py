...
def load_model_for_a1111(timer, checkpoint_info=None, state_dict=None):
    ...
    if conditioner:
        ...
        for i in range(len(conditioner.embedders)):
            ...
            if typename == "FrozenCLIPEmbedder":  # SDXL Clip L
                ...
                embedder.transformer = (
                    forge_objects.clip.cond_stage_model.clip_l.transformer
                )
                ...
                embedder.transformer = (
                    forge_objects.clip.cond_stage_model.clip_g.transformer
                )
                embedder.text_projection = (
                    forge_objects.clip.cond_stage_model.clip_g.text_projection
                )
                model_embeddings = embedder.transformer.text_model.embeddings
                model_embeddings.token_embedding = sd_hijack.EmbeddingsWithFixes(
        ...
        sd_model.cond_stage_model.tokenizer = (
            forge_objects.clip.tokenizer.clip_l.tokenizer
        )
        ...
        sd_model.cond_stage_model.tokenizer = (
            forge_objects.clip.tokenizer.clip_h.tokenizer
        )
        sd_model.cond_stage_model.transformer = (
            forge_objects.clip.cond_stage_model.clip_h.transformer
        )
        model_embeddings = sd_model.cond_stage_model.transformer.text_model.embeddings
        model_embeddings.token_embedding = sd_hijack.EmbeddingsWithFixes(
            model_embeddings.token_embedding, sd_hijack.model_hijack
        ...
        raise NotImplementedError(
            "Bad Clip Class Name:" + type(sd_model.cond_stage_model).__name__
        )
    ...
    sd_model.is_sdxl_inpaint = (
        sd_model.is_sdxl and forge_objects.unet.model.diffusion_model.in_channels == 9
    )
    sd_model.is_sd2 = (
        not sd_model.is_sdxl
        and not is_sd3
        and hasattr(sd_model.cond_stage_model, "model")
    )
    ...
    def patched_decode_first_stage(x):
        sample = (
            sd_model.forge_objects.unet.model.model_config.latent_format.process_out(x)
        )
