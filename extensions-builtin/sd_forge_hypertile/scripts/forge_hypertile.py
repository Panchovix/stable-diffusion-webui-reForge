...
class HyperTileForForge(scripts.Script):
    ...
    def ui(self, *args, **kwargs):
        with gr.Accordion(open=False, label=self.title()):
            ...
            tile_size = gr.Slider(
                label="Tile Size", minimum=1, maximum=2048, step=1, value=256
            )
            swap_size = gr.Slider(
                label="Swap Size", minimum=1, maximum=128, step=1, value=2
            )
            max_depth = gr.Slider(
                label="Max Depth", minimum=0, maximum=10, step=1, value=0
            )
