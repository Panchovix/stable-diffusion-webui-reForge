...
def repeat_to_batch_size(tensor, batch_size, dim=0):
    ...
    if tensor.shape[dim] > batch_size:
        ...
        return tensor.repeat(
            dim * [1]
            + [ceildiv(batch_size, tensor.shape[dim])]
...
def split_bboxes(
    ...
    tile_h: int,
    overlap: int = 16,
    init_weight: Union[Tensor, float] = 1.0,
...
class AbstractDiffusion:
    def __init__(self):
        ...
        self.draw_background: bool = (
            True  # by default we draw major prompts in grid tiles
        )
    ...
    def init_grid_bbox(self, tile_w: int, tile_h: int, overlap: int, tile_bs: int):
        ...
        self.weights = torch.zeros(
            (1, 1, self.h, self.w), device=devices.device, dtype=torch.float32
        )
        ...
        self.batched_bboxes = [
            bboxes[i * self.tile_bs : (i + 1) * self.tile_bs]
    ...
    def get_grid_bbox(
        ...
        h: int,
        device: torch.device,
        get_tile_weights: Callable = lambda: 1.0,
    ) -> List[List[BBox]]:
        ...
        # split the latent into overlapped tiles, then batching
        # weights basically indicate how many times a pixel is painted
        bboxes, weights_ = split_bboxes(
            w, h, tile_w, tile_h, overlap, get_tile_weights()
        )
        weights += weights_
        ...
        batched_bboxes = [
            bboxes[i * tile_bs : (i + 1) * tile_bs] for i in range(num_batches)
        ]
    ...
    @noise_inverse
    def init_noise_inverse(
        self,
        steps: int,
        retouch: float,
        get_cache_callback,
        set_cache_callback,
        renoise_strength: float,
        renoise_kernel: int,
    ):
    ...
    def process_controlnet(
        ...
        while control is not None:
            ...
            if (
                self.refresh
                or control.cond_hint is None
                or not isinstance(
                    self.control_params[tuple_key][param_id][batch_id], Tensor
                )
            ):
                ...
                if dtype is None:
                    dtype = getattr(
                        getattr(control, "control_model", None), "dtype", None
                    )
                if dtype is None:
                    dtype = x_noisy.dtype
                ...
                if isinstance(control, T2IAdapter):
                    ...
                    if (
                        getattr(control, "sub_idxs", None) is not None
                        and control.cond_hint_original.shape[0]
                        >= control.full_latent_length
                    ):
                        cns = common_upscale(
                            control.cond_hint_original[control.sub_idxs],
                            PW,
                            PH,
                            control.upscale_algorithm,
                            "center",
                ...
                cns_slices = [
                    cns[:, :, bbox[1] * cf : bbox[3] * cf, bbox[0] * cf : bbox[2] * cf]
                    for bbox in bboxes
                ]
                ...
                if hasattr(control, "cns") and shifts is not None:
                    ...
                    control.cond_hint = torch.cat(cns_slices, dim=0).to(
                        device=cns.device
                    )
                    del cns_slices
                    del cns
                else:
                    control.cond_hint = self.control_params[tuple_key][param_id][
                        batch_id
                    ]
...
def gaussian_weights(tile_w: int, tile_h: int) -> Tensor:
    ...
    f = lambda x, midpoint, var=0.01: exp(
        -(x - midpoint) * (x - midpoint) / (tile_w * tile_w) / (2 * var)
    ) / sqrt(2 * pi * var)
    x_probs = [
    ...
    def __call__(self, model_function: BaseModel.apply_model, args: dict):
        ...
        if self.draw_background:
            for batch_id, bboxes in enumerate(self.batched_bboxes):
                ...
                x_tile = torch.cat(
                    [x_in[bbox.slicer] for bbox in bboxes], dim=0
                )  # [TB, C, TH, TW]
                    ...
                    if isinstance(v, torch.Tensor):
                        if len(v.shape) == len(x_tile.shape):
                            ...
                            if v.shape[-2:] != x_in.shape[-2:]:
                                cf = (
                                    x_in.shape[-1] * self.compression // v.shape[-1]
                                )  # compression factor
                            ...
                            v = torch.cat(
                                [v[bbox_.slicer] for bbox_ in bboxes_[batch_id]]
                            )
                ...
                if "control" in c_in:
                    self.process_controlnet(
                        x_tile, c_in, cond_or_uncond, bboxes, N, batch_id
                    )
                    c_tile["control"] = c_in["control"].get_control_orig(
                        x_tile, t_tile, c_tile, len(cond_or_uncond)
                    )
...
class SpotDiffusion(AbstractDiffusion):
    ...
    def __call__(self, model_function: BaseModel.apply_model, args: dict):
        ...
        if self.uniform_distribution is None:
            ...
            shift_method = store.model_options.get(
                "tiled_diffusion_shift_method", "random"
            )
            seed = store.model_options.get(
                "tiled_diffusion_seed", store.extra_args.get("seed", 0)
            )
            th = self.tile_height
            tw = self.tile_width
            cf = self.compression
            if "effnet" in c_in:
                cf = (
                    x_in.shape[-1] * self.compression // c_in["effnet"].shape[-1]
                )  # compression factor
                th = self.height // cf
                tw = self.width // cf
            shift_height = torch.randint(
                0,
                th,
                (len(sigmas) - 1,),
            ...
            shift_height = (
                (shift_height * cf / self.compression).round().to(torch.int32)
            )
            shift_width = torch.randint(
                0,
                tw,
                (len(sigmas) - 1,),
                generator=torch.Generator(device="cpu").manual_seed(seed),
        ...
        if min(self.tile_width, x_in.shape[-1]) == x_in.shape[-1]:
            sh_w = 0
        condition = (
            cur_i % 2 == 0 if self.tile_height > self.tile_width else cur_i % 2 != 0
        )
        if (sh_h, sh_w) != (0, 0):
        ...
        if self.draw_background:
            for batch_id, bboxes in enumerate(
                ...
                for i, bbox in enumerate(bboxes):
                    ...
                    self.x_buffer[bbox.slicer] += (
                        x_tile_out[i * N : (i + 1) * N, :, :, :] * w
                    )
                del x_tile_out, x_tile, t_tile, c_tile
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "model": ("MODEL",),
                "method": (
                    ["MultiDiffusion", "Mixture of Diffusers", "SpotDiffusion"],
                ...
                "tile_batch_size": (
                    "INT",
                    {"default": 4, "min": 1, "max": MAX_RESOLUTION, "step": 1},
                ),
            }
        }
