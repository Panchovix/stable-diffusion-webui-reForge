...
class MultiDiffusionForForge(scripts.Script):
        ...
        with gr.Accordion(open=False, label=self.title()):
            ...
            with gr.Row():
                tile_width = gr.Slider(
                    label="Tile Width", minimum=16, maximum=8192, step=16, value=768
                )
                tile_height = gr.Slider(
                    label="Tile Height", minimum=16, maximum=8192, step=16, value=768
                )
            with gr.Row():
                tile_overlap = gr.Slider(
                    label="Tile Overlap", minimum=0, maximum=2048, step=32, value=64
                )
                tile_batch_size = gr.Slider(
                    label="Tile Batch Size", minimum=1, maximum=8192, step=1, value=4
                )
            with gr.Row():
                shift_method = gr.Radio(
                    label="Shift Method (SpotDiffusion)",
    ...
    def process_before_every_sampling(self, p, *script_args, **kwargs):
        ...
        unet = opTiledDiffusion(
            unet, method, tile_width, tile_height, tile_overlap, tile_batch_size
        )[0]
