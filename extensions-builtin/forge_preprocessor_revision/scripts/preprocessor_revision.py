...
from modules_forge.supported_preprocessor import (
    PreprocessorClipVision,
    PreprocessorParameter,
)
from modules_forge.shared import add_supported_preprocessor


def revision_conditioning_modifier(
    ...
    if ignore_prompt:
        for c in cond + uncond:
            c["model_conds"]["c_crossattn"].cond = torch.zeros_like(
...
class PreprocessorClipVisionForRevision(PreprocessorClipVision):
    ...
    def process_before_every_sampling(self, process, cond, mask, *args, **kwargs):
        ...
        unet.model_options["revision_conditions"].append(
            dict(
                cond=cond,
                weight=weight,
                noise_aug=noise_aug,
        ...
        unet.add_conditioning_modifier(
            revision_conditioning_modifier, ensure_uniqueness=True
        )
...
add_supported_preprocessor(
    PreprocessorClipVisionForRevision(
