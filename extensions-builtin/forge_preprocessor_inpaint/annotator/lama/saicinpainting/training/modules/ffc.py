...
from annotator.lama.saicinpainting.training.modules.base import (
    get_activation,
    BaseDiscriminator,
)
from annotator.lama.saicinpainting.training.modules.spatial_transform import (
...
class FFCSE_block(nn.Module):
    ...
    def forward(self, x):
        ...
        x_device = (
            id_l.device
            if torch.is_tensor(id_l)
            else (id_g.device if torch.is_tensor(id_g) else "cpu")
        )
