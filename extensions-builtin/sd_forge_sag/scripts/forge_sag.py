...
def attention_basic_with_sim(q, k, v, heads, mask=None):
    ...
    if attn_precision == torch.float32:
        sim = einsum('b i d, b j d -> b i j', q.to(torch.float32), k.to(torch.float32)) * scale
    else:
...
def patch_SAG(model, scale, blur_sigma, threshold):
    m = model.clone()
    ...
    def process_before_every_sampling(self, p, *script_args, **kwargs):
        ...
        if enabled:
            ...
            if not p.is_hr_pass and p.cfg_scale <= 1:
                ...
                return
            if p.is_hr_pass and p.hr_cfg <= 1:
                print("Self Attention Guidance (hires pass) requires Hires CFG > 1")
                return

            unet = p.sd_model.forge_objects.unet
            unet = patch_SAG(unet, scale, blur_sigma, threshold)
