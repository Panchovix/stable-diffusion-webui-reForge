...
def process_image(image, task_prompt, text_input=None, model_id='microsoft/Florence-2-large'):
    ...
    elif task_prompt == 'Generate Tags':
        task_prompt = '<GENERATE_TAGS>'
        results = run_example(task_prompt, image, model_id=model_id)
        return results, None
    elif task_prompt == 'Mixed Caption':
        task_prompt = '<MIXED_CAPTION>'
        results = run_example(task_prompt, image, model_id=model_id)
        return results, None
    elif task_prompt == 'Mixed Caption Plus':
        task_prompt = '<MIXED_CAPTION_PLUS>'
        results = run_example(task_prompt, image, model_id=model_id)
        return results, None
    elif task_prompt == 'Analyze':
        task_prompt = '<ANALYZE>'
        results = run_example(task_prompt, image, model_id=model_id)
        return results, None
...
miao_task_list = [
    'Generate Tags', 'Caption', 'Detailed Caption', 'More Detailed Caption',  'Mixed Caption', 'Mixed Caption Plus', 'Analyze'
]
...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Tab(label="Florence-2 Image Captioning"):
        with gr.Row():
            with gr.Column():
                input_img = gr.Image(label="Input picture", type="pil", height="40vh")
