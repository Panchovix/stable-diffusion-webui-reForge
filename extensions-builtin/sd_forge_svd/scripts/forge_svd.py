...
from modules_forge.forge_util import (
    numpy_to_pytorch,
    pytorch_to_numpy,
    write_images_to_mp4,
)
from ldm_patched.modules.sd import load_checkpoint_guess_config
from ldm_patched.contrib.external_video_model import (
...
def update_svd_filenames():
    ...
    svd_filenames = [
        pathlib.Path(x).name
        for x in shared.walk_files(
            svd_root, allowed_extensions=[".pt", ".ckpt", ".safetensors"]
        )
    ]
...
def predict(
    ...
    model_raw, _, vae, clip_vision = load_checkpoint_guess_config(
        filename, output_vae=True, output_clip=False, output_clipvision=True
    )
    ...
    output_latent = opKSampler.sample(
        model,
        sampling_seed,
        sampling_steps,
        sampling_cfg,
...
def on_ui_tabs():
    with gr.Blocks() as svd_block:
        with ResizeHandleRow():
            with gr.Column():
                input_image = gr.Image(
                    label="Input Image", source="upload", type="numpy", height=400
                )

                with gr.Row():
                    filename = gr.Dropdown(
                        label="SVD Checkpoint Filename",
                ...
                width = gr.Slider(
                    label="Width", minimum=16, maximum=8192, step=8, value=1024
                )
                height = gr.Slider(
                    label="Height", minimum=16, maximum=8192, step=8, value=576
                )
                video_frames = gr.Slider(
                    label="Video Frames", minimum=1, maximum=4096, step=1, value=14
                )
                motion_bucket_id = gr.Slider(
                    label="Motion Bucket Id", minimum=1, maximum=1023, step=1, value=127
                )
                fps = gr.Slider(label="Fps", minimum=1, maximum=1024, step=1, value=6)
                augmentation_level = gr.Slider(
                ...
                sampling_steps = gr.Slider(
                    label="Sampling Steps", minimum=1, maximum=200, step=1, value=20
                )
                sampling_cfg = gr.Slider(
                    label="CFG Scale", minimum=0.0, maximum=50.0, step=0.1, value=2.5
                )
                sampling_denoise = gr.Slider(
                    label="Sampling Denoise",
                    minimum=0.0,
                    maximum=1.0,
                    step=0.01,
                    value=1.0,
                )
                guidance_min_cfg = gr.Slider(
                    label="Guidance Min Cfg",
                    minimum=0.0,
                    maximum=100.0,
                    step=0.5,
                    value=1.0,
                )
                sampling_sampler_name = gr.Radio(
                    label="Sampler Name",
                    choices=[
                        "euler",
                        "euler_ancestral",
        ...
        generate_button.click(
            predict, inputs=ctrls, outputs=[output_gallery, output_video]
        )
        ...
        parameters_copypaste.add_paste_fields(
            "svd", init_img=input_image, fields=paste_fields
        )
