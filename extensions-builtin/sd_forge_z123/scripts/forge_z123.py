...
from modules.core_ui.common_elements import ToolButton, refresh_symbol
...
def update_model_filenames():
    ...
    model_filenames = [
        ...
        for x in shared.walk_files(
            model_root, allowed_extensions=[".pt", ".ckpt", ".safetensors"]
        )
    ]
...
def predict(
    ...
    model, _, vae, clip_vision = load_checkpoint_guess_config(
            ...
            with gr.Column():
                ...
                with gr.Row():
                    filename = gr.Dropdown(
                        label="Zero123 Checkpoint Filename",
                ...
                width = gr.Slider(
                    label="Width", minimum=16, maximum=8192, step=8, value=256
                )
                height = gr.Slider(
                    label="Height", minimum=16, maximum=8192, step=8, value=256
                )
                batch_size = gr.Slider(
                    label="Batch Size", minimum=1, maximum=4096, step=1, value=4
                )
                elevation = gr.Slider(
                    label="Elevation",
                    minimum=-180.0,
                ...
                sampling_steps = gr.Slider(
                    label="Sampling Steps", minimum=1, maximum=10000, step=1, value=20
                )
                sampling_cfg = gr.Slider(
                    label="CFG Scale", minimum=0.0, maximum=100.0, step=0.1, value=5.0
                )
