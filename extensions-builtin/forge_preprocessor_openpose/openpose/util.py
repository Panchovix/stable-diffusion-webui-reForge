...
def draw_poses(poses: List[HumanPoseResult], H, W, draw_body=True, draw_hand=True, draw_face=True):
    canvas = numpy.zeros(shape=(H, W, 3), dtype=numpy.uint8)
...
def decode_json_as_poses(pose_json: dict) -> Tuple[List[HumanPoseResult], List[AnimalPoseResult], int, int]:
...
def smart_resize(x, s):
    ...
    if Co == 3 or Co == 1:
        ...
        return numpy.stack([smart_resize(x[:, :, i], s) for i in range(Co)], axis=2)
...
def smart_resize_k(x, fx, fy):
    ...
    if Co == 3 or Co == 1:
        ...
        return numpy.stack([smart_resize_k(x[:, :, i], fx, fy) for i in range(Co)], axis=2)
...
def padRightDownCorner(img, stride, padValue):
    ...
    img_padded = img
    pad_up = numpy.tile(img_padded[0:1, :, :]*0 + padValue, (pad[0], 1, 1))
    img_padded = numpy.concatenate((pad_up, img_padded), axis=0)
    pad_left = numpy.tile(img_padded[:, 0:1, :]*0 + padValue, (1, pad[1], 1))
    img_padded = numpy.concatenate((pad_left, img_padded), axis=1)
    pad_down = numpy.tile(img_padded[-2:-1, :, :]*0 + padValue, (pad[2], 1, 1))
    img_padded = numpy.concatenate((img_padded, pad_down), axis=0)
    pad_right = numpy.tile(img_padded[:, -2:-1, :]*0 + padValue, (1, pad[3], 1))
    img_padded = numpy.concatenate((img_padded, pad_right), axis=1)
...
def draw_bodypose(canvas: numpy.ndarray, keypoints: List[Keypoint]) -> numpy.ndarray:
    ...
    Args:
        canvas (numpy.ndarray): A 3D numpy array representing the canvas (image) on which to draw the body pose.
        keypoints (List[Keypoint]): A list of Keypoint objects representing the body keypoints to be drawn.

    Returns:
        numpy.ndarray: A 3D numpy array representing the modified canvas with the drawn body pose.
    ...
    for (k1_index, k2_index), color in zip(limbSeq, colors):
        ...
        Y = numpy.array([keypoint1.x, keypoint2.x]) * float(W)
        X = numpy.array([keypoint1.y, keypoint2.y]) * float(H)
        mX = numpy.mean(X)
        mY = numpy.mean(Y)
...
def draw_handpose(canvas: numpy.ndarray, keypoints: Union[List[Keypoint], None]) -> numpy.ndarray:
    """
    Draw keypoints and connections representing hand pose on a given canvas.

    Args:
        canvas (numpy.ndarray): A 3D numpy array representing the canvas (image) on which to draw the hand pose.
        keypoints (List[Keypoint]| None): A list of Keypoint objects representing the hand keypoints to be drawn
                                          or None if no keypoints are present.

    Returns:
        numpy.ndarray: A 3D numpy array representing the modified canvas with the drawn hand pose.
...
def draw_facepose(canvas: numpy.ndarray, keypoints: Union[List[Keypoint], None]) -> numpy.ndarray:
    """
    Draw keypoints representing face pose on a given canvas.

    Args:
        canvas (numpy.ndarray): A 3D numpy array representing the canvas (image) on which to draw the face pose.
        keypoints (List[Keypoint]| None): A list of Keypoint objects representing the face keypoints to be drawn
                                          or None if no keypoints are present.

    Returns:
        numpy.ndarray: A 3D numpy array representing the modified canvas with the drawn face pose.
