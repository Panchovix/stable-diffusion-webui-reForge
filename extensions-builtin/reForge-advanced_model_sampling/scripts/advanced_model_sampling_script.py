...
class AdvancedModelSamplingScript(scripts.Script):
    ...
    def ui(self, *args, **kwargs):
        with gr.Accordion(open=False, label=self.title()):
            ...
            enabled = gr.Checkbox(
                label="Enable Advanced Model Sampling", value=self.enabled
            )
            ...
            with gr.Group(visible=False) as continuous_v_group:
                continuous_v_sigma_max = gr.Slider(
                    label="Sigma Max",
                    minimum=0.0,
                    maximum=1000.0,
                    step=0.001,
                    value=self.continuous_v_sigma_max,
                )
                continuous_v_sigma_min = gr.Slider(
                    label="Sigma Min",
                    minimum=0.0,
                    maximum=1000.0,
                    step=0.001,
                    value=self.continuous_v_sigma_min,
                )

            with gr.Group(visible=False) as stable_cascade_group:
                stable_cascade_shift = gr.Slider(
                    label="Shift",
                    minimum=0.0,
                    maximum=100.0,
                    step=0.01,
                    value=self.stable_cascade_shift,
                )
            ...
            with gr.Group(visible=False) as sd3_group:
                sd3_shift = gr.Slider(
                    ...
                    maximum=100.0,
                    step=0.01,
                    value=self.sd3_shift,
                )

            with gr.Group(visible=False) as aura_flow_group:
                aura_flow_shift = gr.Slider(
                    label="Shift",
                    minimum=0.0,
                    maximum=100.0,
                    step=0.01,
                    value=self.aura_flow_shift,
                )

            with gr.Group(visible=False) as flux_group:
                flux_max_shift = gr.Slider(
                    label="Max Shift",
                    minimum=0.0,
                    maximum=100.0,
                    step=0.01,
                    value=self.flux_max_shift,
                )
                flux_base_shift = gr.Slider(
                    label="Base Shift",
                    minimum=0.0,
                    maximum=100.0,
                    step=0.01,
                    value=self.flux_base_shift,
                )
            ...
            sampling_mode.change(
                ...
                outputs=[
                    discrete_group,
                    continuous_edm_group,
                    continuous_v_group,
                    stable_cascade_group,
                    sd3_group,
                    aura_flow_group,
                    flux_group,
                ],
        ...
        return (
            ...
            discrete_sampling,
            discrete_zsnr,
            continuous_edm_sampling,
            continuous_edm_sigma_max,
            continuous_edm_sigma_min,
            continuous_v_sigma_max,
            continuous_v_sigma_min,
            stable_cascade_shift,
            sd3_shift,
            aura_flow_shift,
            flux_max_shift,
            flux_base_shift,
    ...
    def process_before_every_sampling(self, p, *args, **kwargs):
        if len(args) >= 16:
            (
                ...
                self.continuous_v_sigma_max,
                self.continuous_v_sigma_min,
                self.stable_cascade_shift,
                self.sd3_shift,
                self.aura_flow_shift,
                self.flux_max_shift,
                self.flux_base_shift,
                self.flux_width,
                self.flux_height,
            ) = args[:16]
        else:
        ...
        if self.sampling_mode == "Discrete":
            unet = ModelSamplingDiscrete().patch(
                unet, self.discrete_sampling, self.discrete_zsnr
            )[0]
            ...
            unet = ModelSamplingContinuousV().patch(
                unet,
                "v_prediction",
                self.continuous_v_sigma_max,
                self.continuous_v_sigma_min,
            ...
            unet = ModelSamplingStableCascade().patch(unet, self.stable_cascade_shift)[
                0
            ]
        ...
        p.extra_generation_params.update(
            {
                ...
                "continuous_edm_sampling": self.continuous_edm_sampling
                if self.sampling_mode == "Continuous EDM"
                else None,
                "continuous_edm_sigma_max": self.continuous_edm_sigma_max
                if self.sampling_mode == "Continuous EDM"
                else None,
                "continuous_edm_sigma_min": self.continuous_edm_sigma_min
                if self.sampling_mode == "Continuous EDM"
                else None,
                "continuous_v_sigma_max": self.continuous_v_sigma_max
                if self.sampling_mode == "Continuous V"
                else None,
                "continuous_v_sigma_min": self.continuous_v_sigma_min
                if self.sampling_mode == "Continuous V"
                else None,
                "stable_cascade_shift": self.stable_cascade_shift
                if self.sampling_mode == "Stable Cascade"
                else None,
                "sd3_shift": self.sd3_shift if self.sampling_mode == "SD3" else None,
                "aura_flow_shift": self.aura_flow_shift
                if self.sampling_mode == "Aura Flow"
                else None,
                "flux_max_shift": self.flux_max_shift
                if self.sampling_mode == "Flux"
                else None,
                "flux_base_shift": self.flux_base_shift
                if self.sampling_mode == "Flux"
                else None,
                "flux_width": self.flux_width if self.sampling_mode == "Flux" else None,
                "flux_height": self.flux_height
                if self.sampling_mode == "Flux"
                else None,
            }
        )
        ...
        logging.debug(
            f"Advanced Model Sampling: Enabled: {self.enabled}, Mode: {self.sampling_mode}"
        )
