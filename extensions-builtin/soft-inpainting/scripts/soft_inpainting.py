...
def apply_adaptive_masks(
    ...
    if len(nmask.shape) == 3:
        latent_mask = nmask[0].to(torch.float32)
    else:
        latent_mask = nmask[:, 0].to(torch.float32)
    ...
    for i, (distance_map, overlay_image) in enumerate(zip(latent_distance, overlay_images)):
        converted_mask = distance_map.to(torch.float32).cpu().numpy()
...
def apply_masks(
    ...
    converted_mask = nmask[0].to(torch.float32)
...
def weighted_histogram_filter(img, kernel, kernel_center, percentile_min=0.0, percentile_max=1.0, min_width=1.0):
    ...
    def weighted_histogram_filter_single(idx):
        ...
        max_index = np.minimum(vec(img.shape), idx + kernel_max)    # min_ max_index are coords of windows around pixel being processed
...
default = SoftInpaintingSettings(1, 0.5, 4, 0, 0.5, 1.5)
