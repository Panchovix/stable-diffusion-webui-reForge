...
def add_cads_noise(
    ...
    if timestep_as_float < cads_schedule_start:
        ...
        gamma = (cads_schedule_end - timestep_as_float) / (
            cads_schedule_end - cads_schedule_start
        )
...
def add_cads_custom_noise(
    ...
    y_mean, y_std = torch.mean(y), torch.std(y)
    y = (
        np.sqrt(gamma) * y + cads_noise_scale * np.sqrt(1 - gamma) * noise
    )  # .sub_(noise.mean()).div_(noise.std())
...
def dyn_thresh_gate(
    latent: Tensor,
    centered_magnitudes: Tensor,
    tonemap_percentile: float,
    floor: float,
    ceil: float,
):
    if (
        centered_magnitudes.lt(torch.tensor(ceil, device=centered_magnitudes.device))
        .all()
        .item()
    ):  # If the magnitudes are less than the ceiling
        return latent  # Return the unmodified centered latent
    else:
        latent = gated_thresholding(
            tonemap_percentile, floor, latent
        )  # If the magnitudes are higher than the ceiling
...
def center_latent(
    tensor,
):  # https://birchlabs.co.uk/machine-learning#combating-mean-drift-in-cfg
    """Centers on 0 to combat CFG drift."""
    tensor = tensor - tensor.mean(dim=(-2, -1)).unsqueeze(-1).unsqueeze(-1).expand(
        tensor.shape
    )
    return tensor
...
def center_latent_perchannel_with_magnitudes(
    tensor,
):  # Does nothing different than above
    """Centers on 0 to combat CFG drift."""
    flattened = tensor.flatten(2)
    flattened_magnitude = (
        torch.linalg.vector_norm(flattened, dim=(2), keepdim=True) + 0.0000000001
    )
...
def center_latent_perchannel_with_decorrelate(
    tensor,
):  # Decorrelates data, slight change, test and play with it.
...
def decorrelate_data(data):
    ...
    cov_matrix = torch.matmul(data_reshaped, data_reshaped.transpose(1, 2)) / (
        height * width - 1
    )

    # Compute the inverse square root of the covariance matrix
    u, s, v = torch.svd(cov_matrix)
    sqrt_inv_cov_matrix = torch.matmul(
        u, torch.matmul(torch.diag_embed(1.0 / torch.sqrt(s)), v.transpose(1, 2))
    )

    # Reshape sqrt_inv_cov_matrix to match the dimensions of data_reshaped
    sqrt_inv_cov_matrix = sqrt_inv_cov_matrix.unsqueeze(0).expand(
        num_samples, -1, -1, -1
    )

    # Decorrelate the data
    decorrelated_data = torch.matmul(
        data_reshaped.transpose(1, 2), sqrt_inv_cov_matrix.transpose(2, 3)
    )
...
def get_low_frequency_noise(image: Tensor, threshold: float):
    ...
    inverse_transformed = torch.fft.ifft2(
        filtered_fourier, dim=(-2, -1)
...
def spectral_modulation(
    image: Tensor, modulation_multiplier: float, spectral_mod_percentile: float
):  # Reference implementation by Clybius, 2023 :tm::c::r: (jk idc who uses it :3)
    # Convert image to Fourier domain
    fourier = torch.fft.fft2(
        image, dim=(-2, -1)
    )  # Apply FFT along Height and Width dimensions
    ...
    quantile_low = (
        torch.quantile(log_amp.abs().flatten(2), spectral_mod_percentile * 0.01, dim=2)
        .unsqueeze(-1)
        .unsqueeze(-1)
    ...
    quantile_high = (
        torch.quantile(
            log_amp.abs().flatten(2), 1 - (spectral_mod_percentile * 0.01), dim=2
        )
        .unsqueeze(-1)
        .unsqueeze(-1)
        .expand(log_amp.shape)
    ...
    mask_low = ((log_amp < quantile_low).float() + 1).clamp_(
        max=1.5
    )  # If lower than low 5% quantile, set to 1.5, otherwise 1
    # Decrease high-frequency components
    mask_high = ((log_amp < quantile_high).float()).clamp_(
        min=0.5
    )  # If lower than high 5% quantile, set to 1, otherwise 0.5
    filtered_fourier = fourier * (
        (mask_low * mask_high) ** modulation_multiplier
    )  # Effectively
...
def spectral_modulation_soft(
    image: Tensor, modulation_multiplier: float, spectral_mod_percentile: float
):  # Modified for soft quantile adjustment using a novel:tm::c::r: method titled linalg.
    # Convert image to Fourier domain
    fourier = torch.fft.fft2(
        image, dim=(-2, -1)
    )  # Apply FFT along Height and Width dimensions
    ...
    quantile_max = (
        torch.quantile(log_amp.abs().flatten(2), 1, dim=2)
        .unsqueeze(-1)
        .unsqueeze(-1)
        .expand(log_amp.shape)
    )
    ...
    mask_mult = (
        (additive_mult_low * additive_mult_high) ** modulation_multiplier
    ).clamp_(min=0.05, max=20)
    # print(mask_mult)
...
def dyn_cfg_modifier(conditioning, unconditioning, method, cond_scale, time_mult):
    match method:
        case "dyncfg-halfcosine":
            ...
            noise_pred_magnitude = (
                torch.linalg.vector_norm(noise_pred, dim=(1)) + 0.0000000001
            )[:, None]

            time = time_mult.item()
            time_factor = -(math.cos(0.5 * time * math.pi) / 2) + 1
            noise_pred_timescaled_magnitude = (
                torch.linalg.vector_norm(noise_pred * time_factor, dim=(1))
                + 0.0000000001
            )[:, None]
        ...
        case "dyncfg-halfcosine-mimic":
            ...
            noise_pred_timescaled_magnitude = (
                torch.linalg.vector_norm(noise_pred_degraded, dim=(1)) + 0.0000000001
            )[:, None]
            noise_pred *= noise_pred_timescaled_magnitude
            return noise_pred
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "sharpness_method": (
                    ["anisotropic", "joint-anisotropic", "gaussian", "cas"],
                ),
                "tonemap_multiplier": (
                    "FLOAT",
                ...
                "tonemap_percentile": (
                    "FLOAT",
                    {"default": 100.0, "min": 0.0, "max": 100.0, "step": 0.005},
                ),
                "contrast_multiplier": (
                    "FLOAT",
                    {"default": 0.0, "min": -100.0, "max": 100.0, "step": 0.1},
                ...
                "combat_method": (
                    ["subtract", "subtract_channels", "subtract_median", "sharpen"],
                ),
                "combat_cfg_drift": (
                    "FLOAT",
                ...
                "extra_noise_type": (
                    ["gaussian", "uniform", "perlin", "pink", "green", "pyramid"],
                ),
                "extra_noise_method": (
                    [
                        "add",
                ...
                "spectral_mod_multiplier": (
                    "FLOAT",
                    {"default": 0.0, "min": -15.0, "max": 15.0, "step": 0.01},
                ),
                "affect_uncond": (["None", "Sharpness"],),
                "dyn_cfg_augmentation": (
                    ["None", "dyncfg-halfcosine", "dyncfg-halfcosine-mimic"],
                ),
            },
        ...
        def modify_latent(args):
            ...
            if extra_noise_multiplier > 0:
                match extra_noise_type:
                    ...
                    case "uniform":
                        extra_noise = (
                            (gen_like(torch.rand, cond, generator=gen) - 0.5) * 2 * 1.73
                        )
                    case "perlin":
                        ...
                        extra_noise = (
                            perlin_noise(
                                grid_shape=(cond_size_0, cond_size_1),
                                out_shape=(cond_size_0, cond_size_1),
                                batch_size=4,
                                generator=gen,
                            )
                            .to(cond.device)
                    ...
                    case "pink":
                        extra_noise = generate_1f_noise(
                            cond, 2, extra_noise_multiplier, generator=gen
                        ).to(cond.device)
                        mean = torch.mean(extra_noise)
                        std = torch.std(extra_noise)
                    ...
                    case "green":
                        cond_size_0 = cond.size(dim=2)
                        cond_size_1 = cond.size(dim=3)
                        extra_noise = green_noise(
                            cond_size_0, cond_size_1, generator=gen
                        ).to(cond.device)
                        mean = torch.mean(extra_noise)
                        std = torch.std(extra_noise)
                ...
                if extra_noise_lowpass > 0:
                    extra_noise = get_low_frequency_noise(
                        extra_noise, extra_noise_lowpass
                    )

                alpha_noise = (
                    1.0 - (timestep / 999.0)[:, None, None, None].clone()
                )  # Get alpha multiplier, lower alpha at high sigmas/high noise
                alpha_noise *= (
                    0.001 * extra_noise_multiplier
                )  # User-input and weaken the strength so we don't annihilate the latent.
                match extra_noise_method:
                    ...
                    case "add_scaled":
                        cond = (
                            cond
                            + train_difference(cond, extra_noise, cond) * alpha_noise
                        )
                        uncond = (
                            uncond
                            - train_difference(uncond, extra_noise, uncond)
                            * alpha_noise
                        )
                    case "speckle":
                    ...
                    case "cads":
                        cond = add_cads_custom_noise(
                            cond,
                            extra_noise,
                            timestep,
                            0.6,
                            0.9,
                            extra_noise_multiplier / 100.0,
                            1,
                            False,
                        )
                        uncond = add_cads_custom_noise(
                            uncond,
                            extra_noise,
                            timestep,
                            0.6,
                    ...
                    case _:
                        print(
                            "Haven't heard of a noise method named like that before... (Couldn't find method)"
                        )
            ...
            if sharpness_multiplier > 0.0 or sharpness_multiplier < 0.0:
                match sharpness_method:
                    ...
                    case "joint-anisotropic":
                        degrade_func = lambda img: joint_bilateral_blur(
                            img,
                            (img - torch.mean(img, dim=(1, 2, 3), keepdim=True))
                            / torch.std(img, dim=(1, 2, 3), keepdim=True),
                            13,
                            3.0,
                            3.0,
                            "reflect",
                    ...
                    case "cas":
                        degrade_func = lambda image: contrast_adaptive_sharpening(
                            image, amount=sigma.clamp(max=1.00).item()
                        )
                    case _:
                        print(
                            "For some reason, the sharpness filter could not be found."
                        )
                # Sharpness
                alpha = (
                    1.0 - (timestep / 999.0)[:, None, None, None].clone()
                ...
                alpha *= (
                    0.001 * sharpness_multiplier
                )  # User-input and weaken the strength so we don't annihilate the latent.
                cond = degrade_func(cond) * alpha + cond * (
                    1.0 - alpha
                )  # Mix the modified latent with the existing latent by the alpha
            ...
            noise_pred_degraded = (
                (cond - uncond)
                if dyn_cfg_augmentation == "None"
                else dyn_cfg_modifier(
                    cond, uncond, dyn_cfg_augmentation, cond_scale, time_mult
                )
            )  # New noise pred
                ...
                match tonemap_method:
                    case "reinhard":
                        noise_pred_vector_magnitude = (
                            torch.linalg.vector_norm(noise_pred_degraded, dim=(1))
                            + 0.0000000001
                        )[:, None]
                        noise_pred_degraded /= noise_pred_vector_magnitude

                        mean = torch.mean(
                            noise_pred_vector_magnitude, dim=(1, 2, 3), keepdim=True
                        )
                        std = torch.std(
                            noise_pred_vector_magnitude, dim=(1, 2, 3), keepdim=True
                        )
                        ...
                        top = (
                            std * 3 * (100 / tonemap_percentile) + mean
                        ) * tonemap_multiplier
                    ...
                    case "reinhard_perchannel":  # Testing the flatten strategy
                        ...
                        noise_pred_vector_magnitude = (
                            torch.linalg.vector_norm(flattened, dim=(2), keepdim=True)
                            + 0.0000000001
                        )
                        flattened /= noise_pred_vector_magnitude

                        mean = torch.mean(
                            noise_pred_vector_magnitude, dim=(2), keepdim=True
                        )

                        top = (
                            3 * (100 / tonemap_percentile) + mean
                        ) * tonemap_multiplier
                        ...
                        noise_pred_degraded = flattened.unflatten(
                            2, noise_pred_degraded.shape[2:]
                        )
                    case "arctan":
                        noise_pred_vector_magnitude = (
                            torch.linalg.vector_norm(noise_pred_degraded, dim=(1))
                        ...
                        noise_pred_degraded = (
                            torch.arctan(noise_pred_degraded * tonemap_multiplier)
                            * (1 / tonemap_multiplier)
                        ) + (noise_pred_degraded * (100 - tonemap_percentile) / 100)

                        noise_pred_degraded *= noise_pred_vector_magnitude
                    case "quantile":
                        s: FloatTensor = (
                            torch.quantile(
                                (uncond + noise_pred_degraded * cond_scale)
                                .flatten(start_dim=1)
                                .abs(),
                    ...
                    case "gated":  # https://birchlabs.co.uk/machine-learning#dynamic-thresholding-latents so based,.,.,....,
                        ...
                        latent = (
                            uncond + noise_pred_degraded * cond_scale
                        )  # Get full latent from CFG formula
                        ...
                        centered_magnitudes = (
                            (flattened - means).abs().max()
                        )  # Get highest magnitude of full CFG

                        flattened_pred = (noise_pred_degraded / latent_scale).flatten(2)

                        floor = 3.0560
                        ceil = (
                            42.0 * tonemap_multiplier
                        )  # as is the answer to life, unless you modify the multiplier cuz u aint a believer in life

                        thresholded_latent = dyn_thresh_gate(
                            flattened_pred,
                            centered_magnitudes,
                            tonemap_percentile / 100.0,
                            floor,
                            ceil,
                        )  # Threshold if passes ceil
                        thresholded_latent = thresholded_latent.unflatten(
                            2, noise_pred_degraded.shape[2:]
                        )
                        noise_pred_degraded = (
                            thresholded_latent * latent_scale
                        )  # Rescale by latent
                    case "cfg-mimic":
                        ...
                        latent_recentered = latent_flattened - latent_means
                        latent_abs = latent_recentered.abs()
                        latent_q = torch.quantile(
                            latent_abs, tonemap_percentile / 100.0, dim=2
                        ).unsqueeze(2)
                        s = torch.maximum(latent_q, mimic_max)
                        ...
                        pred_renorm = pred_normalized * mimic_max
                        pred_uncentered = (
                            pred_renorm + mimic_means
                        )  # Personal choice to re-mean from the mimic here... should be latent_means.
                        noise_pred_degraded = pred_uncentered.unflatten(
                            2, noise_pred_degraded.shape[2:]
                        )
                    case "spatial-norm":
                        # time = (1.0 - (timestep / 999.0)[:, None, None, None].clone().item())
                        # time = -(math.cos(time * math.pi) / (3)) + (2/3) # 0.33333 to 1.0, half cosine
                        noise_pred_degraded = spatial_norm_chw_thresholding(
                            noise_pred_degraded, tonemap_multiplier / 2 / cond_scale
                        )
                    case _:
            ...
            if contrast_multiplier > 0 or contrast_multiplier < 0:
                ...
                noise_pred_degraded = (
                    contrast_func(noise_pred_degraded) * alpha
                    + (noise_pred_degraded) * (1.0 - alpha)
                )  # Temporary fix for contrast is to add the input? Maybe? It just doesn't work like before...
            ...
            if combat_cfg_drift > 0 or combat_cfg_drift < 0:
                ...
                match combat_method:
                    ...
                    case "sharpen":
                        ...
                        alpha *= combat_cfg_drift
                x_final = combat_drift_func(x_final) * alpha + x_final * (
                    1.0 - alpha
                )  # Mix the modified latent with the existing latent by the alpha
            ...
            if divisive_norm_multiplier > 0:
                ...
                high_noise = divisive_normalization(
                    x_final, (divisive_norm_size * 2) + 1
                )
                x_final = high_noise * alpha + x_final * (1.0 - alpha)

            return x_input - (
                x - x_final * sigma / (sigma * sigma + 1.0) ** 0.5
            )  # General formula for CFG. uncond + (cond - uncond) * cond_scale
