...
from k_diffusion import deis, sa_solver
...
def sample_euler_ancestral(model, x, sigmas, extra_args=None, callback=None, disable=None, noise_sampler=None):
    if hasattr(model, 'model_sampling') and isinstance(model.model_sampling, Prediction):
...
def dy_sampling_step_cfg_pp(x, model, sigma_next, i, sigma, sigma_hat, callback, **extra_args):
    ...
    model_options = extra_args.get("model_options", {}).copy()
    extra_args["model_options"] = backend.patcher.base.set_model_options_post_cfg_function(
